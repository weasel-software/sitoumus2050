...
export class ResultService {
  ...
  getResult(id): Observable<any> {
    return this.http.get(
      environment.apiUrl + '/api/extendedResultPopulated/' + id
    );
  }

  createResult(): Observable<any> {
    return this.http.get(environment.apiUrl + '/api/extendedResult');
  }

  updateResult(result): Observable<any> {
    return this.http.post(
      environment.apiUrl + '/api/extendedResult/' + result._id,
      result
    );
  }

  countResults(): Observable<any> {
    return this.http.get(environment.apiUrl + '/api/extendedResults/count');
  }

  saveResult(result): Observable<any> {
    return this.http.post(environment.apiUrl + '/api/average', result);
  }

  getAverages(): Observable<any> {
    return this.http.get(environment.apiUrl + '/api/average');
  }

  getResultCategories(): Observable<any> {
    return this.http.get(environment.apiUrl + '/api/resultCategories');
  }
    ...
    const averageData: Array<any> = [
      {
        index: 1,
        value: 2700,
        icon: '/o/sitoumus-2050-theme/eltest/assets/images/test-icon-asuminen.svg',
        color: 'orange',
      },
      {
        index: 2,
        value: 2000,
        icon: '/o/sitoumus-2050-theme/eltest/assets/images/test-icon-liikenne-ja-matkailu.svg',
        color: 'blue',
      },
      {
        index: 3,
        value: 2100,
        icon: '/o/sitoumus-2050-theme/eltest/assets/images/test-icon-ruoka.svg',
        color: 'green',
      },
      {
        index: 4,
        value: 1000,
        icon: '/o/sitoumus-2050-theme/eltest/assets/images/test-icon-tuotteet-ja-palvelut.svg',
        color: 'yellow',
      },
    ];
